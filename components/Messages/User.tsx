...
type Props = {
    username: string;
    lastMessage: { text: string, sent: string, me: boolean, read: boolean };
}
...
const User = ({ username, lastMessage }: Props) => {
    ...
    return (
        <NextLink href={`/messages/${username}`} className={`flex items-center relative border border-dark hover:bg-dark ${username === router.query.username && "bg-dark"} px-2 py-2 rounded-md`}>
            ...
            <div className={`flex flex-col ml-3 max-w-[250px] ${!lastMessage.me && !lastMessage.read  ? "text-white" : "text-gray-400"}`}>
                <p className="font-semibold hover:text-gray-200">{username}</p>
                <div className="flex items-center space-x-2 ">
                    <p className={`text-sm  text-ellipsis overflow-hidden whitespace-nowrap block`}>{lastMessage.me && "You: "} {lastMessage.text}</p>
                    <p className="text-sm  ">{dayjs(lastMessage.sent).format("HH:mm")}</p>
                </div>
            </div>
            {!lastMessage.me && !lastMessage.read &&
